---
name: bump-version
description: Bump the hdd_health_monitor.Template NuGet package version to cut a new release. Use when the user says "bump the version", "cut a release", "release a new version", "publish a new package version", or similar for this template repo. Edits the version in VerticalSliceArchitecture.nuspec — which is what triggers the publish — but never pushes or opens a PR.
author: Daniel Mackay
---

## What "bump the version" means in this repo

This repo is a `dotnet new` template published to NuGet as `hdd_health_monitor.Template`. The release is push-triggered, not tag-triggered.
